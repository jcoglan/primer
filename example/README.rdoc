...
model from <tt>environment.rn</tt> and just sets up a couple of pages.
...
details, but note that it uses an AMQP bus and does not bind the cache to
the bus: this is because we want individual processes to decide whether
they perform cache expiry.


=== worker.rb

This file just loads <tt>environment.rb</tt>, then binds the Primer cache to
the message bus. This picks up data change notifications from the rest of the
system and makes the cache react to them. You can spin up any number of these
workers to distribute the load: just run
