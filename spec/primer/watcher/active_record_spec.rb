...
describe Primer::Watcher::ActiveRecordMacros do
  ...
  it "publishes a message when an attribute changes" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "name"])
  ...
  it "publishes messages when an object is deleted" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "name"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "age"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "person"])
    @person.destroy
  end
  ...
  it "publishes a message about a belongs_to association when the foreign key changes" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "blog_posts"])
  ...
  it "publishes a message about a belongs_to association when the object changes" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "person_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "person"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @impostor.id, "blog_posts"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "blog_posts"])
  ...
  it "publishes a message when a has_many collection loses a member" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "person_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "title"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "BlogPost", @post.id, "person"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Person", @person.id, "blog_posts"])
    @post.destroy
  end
  ...
  it "publishes a message when an object is pushed to a has_many :through has_many" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @concert.id, "performances"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Artist", @artist.id, "performances"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Artist", @artist.id, "concerts"])
  ...
  it "publishes messages when a join object is deleted from a has_many :through has_many collection" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Performance", @performance.id, "artist_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Performance", @performance.id, "artist"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Performance", @performance.id, "concert_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Performance", @performance.id, "concert"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Performance", @performance.id, "id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Artist", @artist.id, "performances"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Artist", @artist.id, "concerts"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "performances"])
  ...
  it "publishes messages when a member is deleted from a has_many :through has_many collection" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "calendar_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "calendar"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "date"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @festival.id, "venue"])
  ...
  it "publishes messages when a join object is pushed to a has_many :through belongs_to" do
    ...
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @concert.id, "calendar_id"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Concert", @concert.id, "calendar"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Calendar", @calendar.id, "gigs"])
    Primer.cache.should_receive(:changed).with(["ActiveRecord", "Artist", @artist.id, "upcoming_gigs"])
